...
public class ListaEnlazadaGenerica<T> extends ListaGenerica<T> {
	...
	public ListaEnlazadaGenerica<T> clonar() {
		ListaEnlazadaGenerica<T> nueva = new ListaEnlazadaGenerica<T>();
		this.comenzar();
		while (!this.fin()) {
	...
	public boolean agregarTodos(T[] arreglo) {
		boolean b = true; int cont = 0;
		for(int i=0; i<=arreglo.length; i++) {
			if(this.agregarFinal(arreglo[i]) == false) {
				cont ++;
			}
		}
		if(cont!=0) {
